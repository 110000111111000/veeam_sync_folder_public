...
def check_read_permission(directory):
    """Check read permission for a directory."""
    return os.access(directory, os.R_OK)

# Function to check all permissions (read, write, execute)
def check_all_permissions(directory):
    """Check read, write, and execute permissions for a directory."""
    return os.access(directory, os.R_OK) and os.access(directory, os.W_OK) and os.access(directory, os.X_OK)
...
def sync_folders(source_folder, replica_folder, log_file):
    ...
    while True:
        try:
            if not check_read_permission(source_folder):
                print(f"Permission denied: Cannot read from source folder '{source_folder}'")
                return
            if not check_all_permissions(replica_folder):
                print(f"Permission denied: Insufficient permissions for replica folder '{replica_folder}'")
                return
